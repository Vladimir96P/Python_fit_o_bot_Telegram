    ...
    try:
        if msg.text == "Низкая активность 🐌":
            ...
            with psycopg2.connect(db_URL, sslmode="require") as postgre_con:
                ...
                db_obj.execute(f'SELECT sex FROM bot_users_list WHERE "user_id"={msg.from_user.id} ORDER BY to_date("date", 'DD/MM/YYYY') DESC LIMIT 1')
                sex = pretiffy(db_obj.fetchall())
                db_obj.execute(f'SELECT age FROM bot_users_list WHERE "user_id"={msg.from_user.id} ORDER BY to_date("date", 'DD/MM/YYYY') DESC LIMIT 1')
                age = int(pretiffy(db_obj.fetchall()))
                db_obj.execute(f'SELECT height FROM bot_users_list WHERE "user_id"={msg.from_user.id} ORDER BY to_date("date", 'DD/MM/YYYY') DESC LIMIT 1')
...
def callback_worker(call):
    if call.text == "Хочу сообщить свой текущий вес 😱":
        ...
        \nНужен следующий формат: dd-mm-yyyy 85.4 где первая часть - дата (например: 31-12-2021), а вторая - вес.''')
