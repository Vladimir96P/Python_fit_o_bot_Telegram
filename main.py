...
def send_welcome(message):
    msg = bot.reply_to(message, "Хэй\! Меня зовут Фитобот😊 А тебя как? Напиши, пожалуйста: свое *имя*, *возраст*, *рост* \(в сантиметрах\) и \(пол* \(русскоязычной раскладкой м/ж\) на моем примере \(обязательно следуй формату, если с первого раза не получится \- вызови повторно команду start и повтори попытку, данные пишем через пробел\): *Фитобот 25 175 М*", parse_mode="MarkdownV2")
...
def delete_last(msg):
    ...
    with psycopg2.connect(db_URL, sslmode="require") as postgre_con:
        db_obj = postgre_con.cursor()
        db_obj.execute(f'DELETE FROM bot_users_weights_table WHERE "user_id"={msg.from_user.id} ORDER BY ID DESC LIMIT 1')
        postgre_con.commit()
        db_obj.close()
        bot.send_message(msg.chat.id, 'Прошло успешно!')
        send_keyboard(msg, "Чем еще могу помочь?")
